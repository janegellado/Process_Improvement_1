...
class SalesAgent extends CI_Controller {
	...
	public function index()
	{
        if($this->session->userdata('userID')){
            ...
            $this->load->view('management_dashboard_user',$data);
    ...
    public function viewSalesReport(){
        ...
        if($_POST['range']=='week'){
            ...
            echo $this->load->view('sales_agent_salesreport',$data,TRUE);
		}
           ...
           echo $this->load->view('sales_agent_salesreport',$data,TRUE);
			...
			$cond = "userID='$userID'";
            $condition = "status='Purchased' AND date='$date'";
			$orders = $this->Order->read($cond);
			$total = 0;
				if($orders!=false){
						foreach($orders as $o){
    ...
    public function delOrder($orderID){
        ...
        $trans = $this->Transaction->read($condition);
      	 foreach($trans as $s)
      	 {
      	 	$where_array = array('transID'=>$s['transID']);
